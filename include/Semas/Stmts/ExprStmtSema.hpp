...
namespace Ace
{
    class ExprStmtSema : public std::enable_shared_from_this<ExprStmtSema>,
    ...
    {
    public:
        ExprStmtSema(const SrcLocation& srcLocation, const std::shared_ptr<const IExprSema>& expr);
        ...
        auto CreateTypeChecked(const StmtTypeCheckingContext& context) const
            -> Diagnosed<std::shared_ptr<const ExprStmtSema>> final;
        ...
        auto CreateLowered(const LoweringContext& context) const
            -> std::shared_ptr<const ExprStmtSema> final;
