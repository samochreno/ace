...
namespace Ace
{
    ...
    {
    public:
        BlockEndStmtSema(const SrcLocation& srcLocation, const std::shared_ptr<Scope>& bodyScope);
        ...
        auto CreateTypeChecked(const StmtTypeCheckingContext& context) const
            -> Diagnosed<std::shared_ptr<const BlockEndStmtSema>> final;
        ...
        auto CreateLowered(const LoweringContext& context) const
            -> std::shared_ptr<const BlockEndStmtSema> final;
