...
namespace Ace
{
    class OrExprBoundNode :
        ...
        public virtual ICloneableWithDiagnosticsBoundNode<OrExprBoundNode>,
    ...
    public:
        ...
        ) const -> Expected<Cacheable<std::shared_ptr<const OrExprBoundNode>>> final;
        auto GetOrCreateTypeCheckedExpr(
            const TypeCheckingContext& context
        ) const -> Expected<Cacheable<std::shared_ptr<const IExprBoundNode>>> final;
        auto GetOrCreateLowered(
            const LoweringContext& context
        ) const -> Cacheable<std::shared_ptr<const OrExprBoundNode>> final;
