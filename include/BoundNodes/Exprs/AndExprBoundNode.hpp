...
namespace Ace
{
    class AndExprBoundNode :
        ...
        public virtual ICloneableWithDiagnosticsBoundNode<AndExprBoundNode>,
    ...
    public:
        ...
        ) const -> Expected<Cacheable<std::shared_ptr<const AndExprBoundNode>>> final;
        auto GetOrCreateTypeCheckedExpr(
            const TypeCheckingContext& context
        ) const -> Expected<Cacheable<std::shared_ptr<const IExprBoundNode>>> final;
        auto GetOrCreateLowered(
            const LoweringContext& context
        ) const -> Cacheable<std::shared_ptr<const AndExprBoundNode>> final;
