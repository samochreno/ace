...
namespace Ace
{
    class AddressOfExprBoundNode :
        ...
        public virtual ICloneableWithDiagnosticsBoundNode<AddressOfExprBoundNode>,
    ...
    public:
        ...
        ) const -> std::shared_ptr<const AddressOfExprBoundNode> final;
        auto CloneWithDiagnosticsExpr(
            DiagnosticBag diagnostics
        ) const -> std::shared_ptr<const IExprBoundNode> final;
        auto GetOrCreateTypeChecked(
            const TypeCheckingContext& context
        ) const -> Expected<Cacheable<std::shared_ptr<const AddressOfExprBoundNode>>> final;
        auto GetOrCreateTypeCheckedExpr(
            const TypeCheckingContext& context
        ) const -> Expected<Cacheable<std::shared_ptr<const IExprBoundNode>>> final;
        auto GetOrCreateLowered(
            const LoweringContext& context
        ) const -> Cacheable<std::shared_ptr<const AddressOfExprBoundNode>> final;
