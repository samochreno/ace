...
namespace Ace
{
    ...
    public:
        ...
        ) const -> Expected<Cacheable<std::shared_ptr<const NormalAssignmentStmtBoundNode>>> final;
        auto GetOrCreateTypeCheckedStmt(
            const StmtTypeCheckingContext& context
        ) const -> Expected<Cacheable<std::shared_ptr<const IStmtBoundNode>>> final;
        auto GetOrCreateLowered(
            const LoweringContext& context
        ) const -> Cacheable<std::shared_ptr<const NormalAssignmentStmtBoundNode>> final;
