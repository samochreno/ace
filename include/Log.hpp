...
namespace Ace
{
    ...
    {
        LoggerConfiguration(std::ostream& stream)
            : Stream{ stream }
        {
        }
    ...
    {
    public:
        Logger(const LoggerConfiguration configuration)
            : m_Configuration{ configuration }
        {
        }

        template <typename T> auto operator<<(const T& value) const -> const Logger&
        {
