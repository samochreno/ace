...
{
    template <typename T>
    inline constexpr bool IsDiagnosedVoid = requires(T t) { t._DiagnosedVoid(); };

    template <typename T>
    inline constexpr bool IsDiagnosedNotVoid = requires(T t) { t._DiagnosedNotVoid(); };

    template <typename T>
    inline constexpr bool IsExpectedVoid = requires(T t) { t._ExpectedVoid(); };

    template <typename T>
    inline constexpr bool IsExpectedNotVoid = requires(T t) { t._ExpectedNotVoid(); };
    ...
    public:
        ...
        auto Collect(TDiagnosed value) -> void
            requires IsDiagnosedVoid<TDiagnosed>
        {
            Add(std::move(value.GetDiagnostics()));
        }

        template <typename TDiagnosed>
        [[nodiscard]] auto Collect(TDiagnosed value)
            requires IsDiagnosedNotVoid<TDiagnosed>
        {
            Add(std::move(value.GetDiagnostics()));
        ...
        auto Collect(TExpected value) -> bool
            requires IsExpectedVoid<TExpected>
        {
            Add(std::move(value.GetDiagnostics()));
            return value;
        }
        ...
        template <typename TExpected>
        [[nodiscard]] auto Collect(TExpected value)
            requires IsExpectedNotVoid<TExpected>
        {
            Add(std::move(value.GetDiagnostics()));
            if (!value)
            {
            ...
            return std::optional<std::decay_t<decltype(value.Unwrap())>>{ std::move(value.Unwrap()
            ) };
        }
