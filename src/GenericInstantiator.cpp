...
namespace Ace
{
    GenericInstantiator::GenericInstantiator(Scope* const scope)
        : m_Scope{ scope }
    {
    }
    ...
    static auto IsSpecialGenericInstance(IGenericSymbol* const root) -> bool
    {
        ...
        return (root == natives.Ref.TryGetSymbol()) || (root == natives.WeakPtr.TryGetSymbol()) ||
    ...
    ) -> Diagnosed<void>
    {
        ...
        std::for_each(
            ...
            [&](ITypeSymbol* const typeArg)
            {
                ...
                {
                    diagnostics.Add(CreateUnsizedTypeArgError(srcLocation, typeArg));
                }
            }
        );
    ...
    ) -> void
    {
        ...
        auto ownedInstantiatedSymbol = symbol->CreateInstantiated(instantiatedScope, context);
    ...
    ) -> void
    {
        ...
        std::map<std::shared_ptr<Scope>, std::shared_ptr<Scope>>
        ...
        std::for_each(
            ...
            [&](ISymbol* const symbol)
            {
                ...
                InstantiateScopeSymbol(
                    scope, instantiatedScope, context, symbol, originalToInstantiatedBodyScopeMap
                );
            }
        );
        ...
        std::for_each(
            ...
            {
                const auto instantiatedChildScope =
                    originalToInstantiatedBodyScopeMap.contains(childScope)
                        ? originalToInstantiatedBodyScopeMap.at(childScope)
                        : instantiatedScope->CreateChild();

                InstantiateScopeSymbolsAndChildren(childScope, instantiatedChildScope, context);
            }
        );
    }
    ...
    ) -> Expected<ISymbol*>
    {
        ...
        diagnostics.Collect(DiagnoseUnsizedTypeArgs(srcLocation, root, context.TypeArgs));
        ...
        {
            diagnostics.Collect(
                constrained->DiagnoseUnsatisfiedConstraints(srcLocation, context.TypeArgs)
            );
            if (diagnostics.HasErrors())
            {
                return std::move(diagnostics);
        ...
        auto ownedInstance = root->CreateInstantiated(root->GetScope(), context);

        auto* const instance = dynamic_cast<IGenericSymbol*>(ownedInstance.get());
        ACE_ASSERT(instance);

        self.m_InstanceSet.insert(instance);
        self.m_InstanceToInstantiationSrcLocationMap[instance] = srcLocation;

        (void)DiagnosticBag::CreateNoError().Collect(Scope::DeclareSymbol(std::move(ownedInstance))
        );
    ...
    static auto IsInstantiatable(IGenericSymbol* const generic) -> bool
    {
        ...
        return prototype && (prototype->GetSelfType() == prototype->CollectSelfType().value());
    }
    ...
    auto GenericInstantiator::GetGenericRoot(const IGenericSymbol* const instance)
        -> IGenericSymbol*
    {
        const auto* const unaliasedInstance = instance->GetUnaliased();

        const auto& self = unaliasedInstance->GetScope()->GetGenericInstantiator();

        const auto it = self.m_NameToRootMap.find(unaliasedInstance->GetName().String);
        ...
        while (true)
        {
            ...
            std::for_each(
                ...
                [&](IGenericSymbol* const instance)
                {
                    ...
                    const auto& monos = self.m_RootToMonosMap.at(instance->GetGenericRoot());
    ...
    auto GenericInstantiator::DeferOrInstantiateBody(IGenericSymbol* const instance) -> void
    {
        auto& self = instance->GetScope()->GetGenericInstantiator();
    ...
    auto GenericInstantiator::InstantiateBody(IGenericSymbol* const instance) -> void
    {
        auto& self = instance->GetScope()->GetGenericInstantiator();
    ...
    ) -> Diagnosed<void>
    {
        ...
        std::for_each(
            begin(functionBlockBindings),
            end(functionBlockBindings),
            [&](const FunctionBlockBinding& functionBlockBinding)
            {
                ...
                diagnostics.Collect(
                    Application::CreateAndBindFunctionBodies({ functionBlockBinding })
                );
    ...
    auto GenericInstantiator::CollectAndClearDeferredInstances() -> std::vector<IGenericSymbol*>
    {
        ...
        std::for_each(
            ...
            [&](const std::shared_ptr<Scope>& childScope)
            {
                ...
                instances.insert(end(instances), begin(childInstances), end(childInstances));
            }
        );
    ...
    auto GenericInstantiator::CollectAndClearReferencedMonosInstances()
        -> std::vector<IGenericSymbol*>
    {
