...
namespace Ace
{
    ...
    auto WhileStmtSema::CreateTypeChecked(const StmtTypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const WhileStmtSema>>
    {
        ...
        const auto checkedBlock =
            diagnostics.Collect(m_Block->CreateTypeChecked({ context.ParentFunctionTypeSymbol }));

        if ((checkedCondition == m_Condition) && (checkedBlock == m_Block))
        {
        ...
        return Diagnosed{
            std::make_shared<const WhileStmtSema>(
                GetSrcLocation(), m_Scope, checkedCondition, checkedBlock
            ),
            std::move(diagnostics),
        };
    }

    auto WhileStmtSema::CreateTypeCheckedStmt(const StmtTypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IStmtSema>>
    {
        return CreateTypeChecked(context);
    }

    auto WhileStmtSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const GroupStmtSema>
    {
        ...
        auto beginLabelSymbolOwned = std::make_unique<LabelSymbol>(m_Scope, beginLabelName);

        auto* const beginLabelSymbol =
        ...
        auto continueLabelSymbolOwned = std::make_unique<LabelSymbol>(m_Scope, continueLabelName);

        auto* const continueLabelSymbol =
            dynamic_cast<LabelSymbol*>(DiagnosticBag::CreateNoError().Collect(
        ...
        stmts.push_back(std::make_shared<const NormalJumpStmtSema>(
            GetSrcLocation().CreateFirst(), m_Scope, continueLabelSymbol
        ));

        stmts.push_back(std::make_shared<const LabelStmtSema>(
            beginLabelSymbol->GetName().SrcLocation, beginLabelSymbol
        ));

        stmts.push_back(m_Block);

        stmts.push_back(std::make_shared<const LabelStmtSema>(
            continueLabelSymbol->GetName().SrcLocation, continueLabelSymbol
        ));

        stmts.push_back(std::make_shared<const ConditionalJumpStmtSema>(
            m_Condition->GetSrcLocation(), m_Condition, beginLabelSymbol
        ));
    ...
    auto WhileStmtSema::CreateLoweredStmt(const LoweringContext& context) const
    ...
    auto WhileStmtSema::CreateControlFlowInstructions() const -> std::vector<ControlFlowInstruction>
    {
        ACE_UNREACHABLE();
    }
}
