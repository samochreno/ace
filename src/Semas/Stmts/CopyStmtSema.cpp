...
namespace Ace
{
    ...
    auto CopyStmtSema::CreateTypeChecked(const StmtTypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const CopyStmtSema>>
    {
        auto diagnostics = DiagnosticBag::Create();

        const auto checkedSrcExpr = diagnostics.Collect(m_SrcExpr->CreateTypeCheckedExpr({}));
        const auto checkedDstExpr = diagnostics.Collect(m_DstExpr->CreateTypeCheckedExpr({}));

        auto* const ptrTypeSymbol = GetCompilation()->GetNatives().Ptr.GetSymbol();

        if (checkedSrcExpr->GetTypeInfo().Symbol != ptrTypeSymbol)
        {
            diagnostics.Add(CreateExpectedPtrError(checkedSrcExpr->GetSrcLocation()));
        }

        if (checkedDstExpr->GetTypeInfo().Symbol != ptrTypeSymbol)
        {
            diagnostics.Add(CreateExpectedPtrError(checkedDstExpr->GetSrcLocation()));
        }

        if ((checkedSrcExpr == m_SrcExpr) && (checkedDstExpr == m_DstExpr))
        {
        ...
        return Diagnosed{
            std::make_shared<const CopyStmtSema>(
                m_SrcLocation, m_TypeSymbol, checkedSrcExpr, checkedDstExpr
            ),
            std::move(diagnostics),
        };
    }

    auto CopyStmtSema::CreateTypeCheckedStmt(const StmtTypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IStmtSema>>
    {
        return CreateTypeChecked(context);
    }

    auto CopyStmtSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const CopyStmtSema>
    {
        ...
        if ((loweredSrcExpr == m_SrcExpr) && (loweredDstExpr == m_DstExpr))
        {
            return shared_from_this();
        }

        return std::make_shared<const CopyStmtSema>(
            m_SrcLocation, m_TypeSymbol, loweredSrcExpr, loweredDstExpr
        );
    }

    auto CopyStmtSema::CreateLoweredStmt(const LoweringContext& context) const
    ...
    {
        return MonoCollector{}.Collect(m_SrcExpr).Collect(m_DstExpr);
    }
    ...
    auto CopyStmtSema::Emit(Emitter& emitter) const -> void
    {
        ...
        tmps.insert(end(tmps), begin(srcExprEmitResult.Tmps), end(srcExprEmitResult.Tmps));

        const auto dstExprEmitResult = m_DstExpr->Emit(emitter);
        tmps.insert(end(tmps), begin(dstExprEmitResult.Tmps), end(dstExprEmitResult.Tmps));
        ...
        emitter.GetBlock().Builder.CreateCall(
            emitter.GetFunction(glueSymbol), { dstExprEmitResult.Value, srcExprEmitResult.Value }
        );
    }

    auto CopyStmtSema::CreateControlFlowInstructions() const -> std::vector<ControlFlowInstruction>
    {
        return {};
    }
}
