...
namespace Ace
{
    ...
    auto AssertStmtSema::CreateTypeChecked(const StmtTypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const AssertStmtSema>>
    {
        ...
        return Diagnosed{
            std::make_shared<const AssertStmtSema>(GetSrcLocation(), checkedCondition),
            std::move(diagnostics),
        };
    }

    auto AssertStmtSema::CreateTypeCheckedStmt(const StmtTypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IStmtSema>>
    {
        return CreateTypeChecked(context);
    }

    auto AssertStmtSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const GroupStmtSema>
    {
        ...
        const auto condition = std::make_shared<const LogicalNegationExprSema>(
            loweredCondition->GetSrcLocation(), loweredCondition
        );

        const auto blockScope = GetScope()->CreateChild();

        const auto exitStmt = std::make_shared<const ExitStmtSema>(GetSrcLocation(), blockScope);

        const auto blockStmt = std::make_shared<const BlockStmtSema>(
            GetSrcLocation(), blockScope, std::vector<std::shared_ptr<const IStmtSema>>{ exitStmt }
        );
    ...
    auto AssertStmtSema::CreateLoweredStmt(const LoweringContext& context) const
    ...
    auto AssertStmtSema::CreateControlFlowInstructions() const
