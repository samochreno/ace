...
namespace Ace
{
    auto IExpandableStmtSema::CreateExpanded() const
        -> std::vector<std::shared_ptr<const IStmtSema>>
    {
        ...
        std::for_each(
            begin(partiallyExpanded),
            end(partiallyExpanded),
            [&](const std::shared_ptr<const IStmtSema>& stmt)
            {
                if (const auto* const expandable =
                        dynamic_cast<const IExpandableStmtSema*>(stmt.get()))
                {
