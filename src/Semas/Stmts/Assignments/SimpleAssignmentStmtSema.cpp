...
namespace Ace
{
    ...
    auto SimpleAssignmentStmtSema::CreateTypeChecked(const StmtTypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const SimpleAssignmentStmtSema>>
    {
        auto diagnostics = DiagnosticBag::Create();

        auto* const lhsExprTypeSymbol = m_LHSExpr->GetTypeInfo().Symbol->GetWithoutRef();

        const auto checkedLHSExpr = diagnostics.Collect(CreateImplicitlyConvertedAndTypeChecked(
            m_LHSExpr, TypeInfo{ lhsExprTypeSymbol, ValueKind::L }
        ));
        const auto checkedRHSExpr = diagnostics.Collect(CreateImplicitlyConvertedAndTypeChecked(
            m_RHSExpr, TypeInfo{ lhsExprTypeSymbol, ValueKind::R }
        ));
        ...
        return Diagnosed{
            std::make_shared<const SimpleAssignmentStmtSema>(
                GetSrcLocation(), checkedLHSExpr, checkedRHSExpr
            ),
            std::move(diagnostics),
        };
    }

    auto SimpleAssignmentStmtSema::CreateTypeCheckedStmt(const StmtTypeCheckingContext& context
    ) const -> Diagnosed<std::shared_ptr<const IStmtSema>>
    {
        return CreateTypeChecked(context);
    }

    auto SimpleAssignmentStmtSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const SimpleAssignmentStmtSema>
    {
        ...
        return std::make_shared<const SimpleAssignmentStmtSema>(
                   GetSrcLocation(), loweredLHSExpr, loweredRHSExpr
        )
            ->CreateLowered({});
    }

    auto SimpleAssignmentStmtSema::CreateLoweredStmt(const LoweringContext& context) const
    ...
    auto SimpleAssignmentStmtSema::Emit(Emitter& emitter) const -> void
    {
        ...
        emitter.EmitCopy(lhsEmitResult.Value, rhsEmitResult.Value, m_LHSExpr->GetTypeInfo().Symbol);

        emitter.EmitDropTmps(tmps);
    }

    auto SimpleAssignmentStmtSema::CreateControlFlowInstructions() const
