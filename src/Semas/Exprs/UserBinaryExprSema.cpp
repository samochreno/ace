...
namespace Ace
{
    ...
    auto UserBinaryExprSema::CreateTypeChecked(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const UserBinaryExprSema>>
    {
        ...
        if (!m_OpSymbol->IsError())
        {
            ...
            convertedLHSExpr =
                diagnostics.Collect(CreateImplicitlyConverted(m_LHSExpr, argTypeInfos.at(0)));
            convertedRHSExpr =
                diagnostics.Collect(CreateImplicitlyConverted(convertedRHSExpr, argTypeInfos.at(1))
                );
        }
    ...
    auto UserBinaryExprSema::CreateTypeCheckedExpr(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        return CreateTypeChecked(context);
    }

    auto UserBinaryExprSema::CreateLowered(const LoweringContext& context) const
    ...
    auto UserBinaryExprSema::CreateLoweredExpr(const LoweringContext& context) const
