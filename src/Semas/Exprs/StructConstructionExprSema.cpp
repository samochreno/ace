...
namespace Ace
{
    ...
    auto StructConstructionExprSema::CreateTypeChecked(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const StructConstructionExprSema>>
    {
        ...
        std::transform(
            ...
            [&](const StructConstructionExprSemaArg& arg)
            {
                const auto checkedValue = diagnostics.Collect(arg.Value->CreateTypeCheckedExpr({}));
        ...
        return Diagnosed{
            std::make_shared<const StructConstructionExprSema>(
                GetSrcLocation(), GetScope(), m_StructSymbol, checkedArgs
            ),
            std::move(diagnostics),
        };
    }

    auto StructConstructionExprSema::CreateTypeCheckedExpr(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        return CreateTypeChecked(context);
    }

    auto StructConstructionExprSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const StructConstructionExprSema>
    {
        ...
        return std::make_shared<const StructConstructionExprSema>(
                   GetSrcLocation(), GetScope(), m_StructSymbol, loweredArgs
        )
            ->CreateLowered({});
    }

    auto StructConstructionExprSema::CreateLoweredExpr(const LoweringContext& context) const
    ...
    auto StructConstructionExprSema::Emit(Emitter& emitter) const -> ExprEmitResult
    {
        std::vector<ExprDropInfo> tmps{};

        auto* const structType = emitter.GetType(m_StructSymbol);

        auto* const allocaInst = emitter.GetBlock().Builder.CreateAlloca(structType);
        ...
        std::for_each(
            ...
            [&](const StructConstructionExprSemaArg& arg)
            {
                ...
                auto* const elementPtr =
                    emitter.GetBlock().Builder.CreateStructGEP(structType, allocaInst, varIndex);
