...
namespace Ace
{
    ...
    static auto GetRawExprTypeSymbol(const std::shared_ptr<const IExprSema>& expr) -> ITypeSymbol*
    {
        return expr->GetTypeInfo().Symbol->GetWithoutRef()->GetWithoutWeakPtr();
    }

    static auto GetLockSymbol(ITypeSymbol* const rawExprTypeSymbol) -> FunctionSymbol*
    {
        ...
        return (sizedRawExprTypeSymbol ? natives.weak_ptr_lock : natives.weak_ptr_lock_dyn)
            .GetSymbol();
    }

    auto LockExprSema::CreateTypeChecked(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const LockExprSema>>
    {
        ...
        auto* const symbol = Scope::ForceCollectGenericInstance(
            GetLockSymbol(rawExprTypeSymbol), { rawExprTypeSymbol }
        );
    ...
    auto LockExprSema::CreateTypeCheckedExpr(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        return CreateTypeChecked(context);
    }

    auto LockExprSema::CreateLowered(const LoweringContext& context) const
    ...
    auto LockExprSema::CreateLoweredExpr(const LoweringContext& context) const
    ...
    {
        return { GetRawExprTypeSymbol(m_Expr)->GetWithAutoStrongPtr(), ValueKind::R };
    }
}
