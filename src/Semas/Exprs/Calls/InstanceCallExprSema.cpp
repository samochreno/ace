...
namespace Ace
{
    ...
    static auto IsFunctionNotDynDispatchable(
        const std::shared_ptr<const IExprSema>& expr, ICallableSymbol* const callableSymbol
    ) -> bool
    {
        ...
        const bool isSelfDyn = expr->GetTypeInfo().Symbol->GetWithoutRef()->IsDynStrongPtr();
        if (!isSelfDyn)
        {
            return false;
        }
    ...
    static auto DiagnoseMismatchedSelfExprType(
        const std::shared_ptr<const IExprSema>& expr, SelfParamVarSymbol* const paramSymbol
    ) -> Diagnosed<void>
    {
        auto diagnostics = DiagnosticBag::Create();

        const bool isParamStrongPtr = paramSymbol->GetType()->GetWithoutRef()->IsAnyStrongPtr();

        const bool isExprStrongPtr = expr->GetTypeInfo().Symbol->GetWithoutRef()->IsAnyStrongPtr();

        if (isParamStrongPtr && !isExprStrongPtr)
        {
            diagnostics.Add(CreateMismatchedSelfExprTypeError(expr->GetSrcLocation(), paramSymbol));
        }

        return Diagnosed<void>{ std::move(diagnostics) };
    }

    auto InstanceCallExprSema::CreateTypeChecked(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const InstanceCallExprSema>>
    {
        auto diagnostics = DiagnosticBag::Create();
        ...
        if (!m_CallableSymbol->IsError())
        {
            ...
            diagnostics.Collect(DiagnoseMismatchedSelfExprType(
                checkedExpr, m_CallableSymbol->CollectSelfParam().value()
            ));

            diagnostics.Collect(
                DiagnoseFunctionNotDynDispatchable(GetSrcLocation(), checkedExpr, m_CallableSymbol)
            );
        }
        ...
        if ((checkedExpr == m_Expr) && (checkedArgs == m_Args))
        {
        ...
        return Diagnosed{
            std::make_shared<const InstanceCallExprSema>(
                GetSrcLocation(), checkedExpr, m_CallableSymbol, checkedArgs
            ),
            std::move(diagnostics),
        };
    }

    auto InstanceCallExprSema::CreateTypeCheckedExpr(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        return CreateTypeChecked(context);
    }

    auto InstanceCallExprSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const InstanceCallExprSema>
    {
        ...
        if ((loweredExpr == m_Expr) && (loweredArgs == m_Args))
        {
            return shared_from_this();
        }

        return std::make_shared<const InstanceCallExprSema>(
                   GetSrcLocation(), loweredExpr, m_CallableSymbol, loweredArgs
        )
            ->CreateLowered({});
    }

    auto InstanceCallExprSema::CreateLoweredExpr(const LoweringContext& context) const
        -> std::shared_ptr<const IExprSema>
    {
        return CreateLowered(context);
    }

    static auto CreateDerefed(const std::shared_ptr<const IExprSema>& expr)
        -> std::shared_ptr<const IExprSema>
    {
    ...
    static auto CreateDerefedNormalSelfExpr(const std::shared_ptr<const IExprSema>& expr)
        -> std::shared_ptr<const IExprSema>
    {
        auto* const typeSymbol = expr->GetTypeInfo().Symbol;
        ...
        return std::make_shared<const RefExprSema>(expr->GetSrcLocation(), expr);
    }

    static auto CreateDerefedStrongPtrSelfExpr(const std::shared_ptr<const IExprSema>& expr)
        -> std::shared_ptr<const IExprSema>
    {
        auto* const typeSymbol = expr->GetTypeInfo().Symbol;
    ...
    {
        return MonoCollector{}.Collect(m_Expr).Collect(m_CallableSymbol).Collect(m_Args);
    }

    auto InstanceCallExprSema::Emit(Emitter& emitter) const -> ExprEmitResult
    {
        ...
        const auto selfExpr =
            CreateDerefedSelfExpr(m_Expr, m_CallableSymbol->CollectSelfParam().value());
        auto* const selfType = emitter.GetType(selfExpr->GetTypeInfo().Symbol);
        const auto selfEmitResult = selfExpr->Emit(emitter);
        tmps.insert(end(tmps), begin(selfEmitResult.Tmps), end(selfEmitResult.Tmps));
