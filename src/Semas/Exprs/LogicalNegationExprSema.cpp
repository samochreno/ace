...
namespace Ace
{
    ...
    auto LogicalNegationExprSema::CreateTypeChecked(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const LogicalNegationExprSema>>
    {
        ...
        return Diagnosed{
            std::make_shared<const LogicalNegationExprSema>(GetSrcLocation(), checkedExpr),
            std::move(diagnostics),
        };
    }

    auto LogicalNegationExprSema::CreateTypeCheckedExpr(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        return CreateTypeChecked(context);
    }

    auto LogicalNegationExprSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const LogicalNegationExprSema>
    {
        ...
        return std::make_shared<const LogicalNegationExprSema>(GetSrcLocation(), loweredExpr)
            ->CreateLowered({});
    }

    auto LogicalNegationExprSema::CreateLoweredExpr(const LoweringContext& context) const
    ...
    auto LogicalNegationExprSema::Emit(Emitter& emitter) const -> ExprEmitResult
    {
        ...
        auto* const loadInst =
            emitter.GetBlock().Builder.CreateLoad(boolType, exprEmitResult.Value);

        auto* const negatedValue = emitter.GetBlock().Builder.CreateXor(loadInst, 1);
