...
namespace Ace
{
    ...
    auto LiteralExprSema::CreateTypeChecked(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const LiteralExprSema>>
    {
        return Diagnosed{ shared_from_this(), DiagnosticBag::Create() };
    }

    auto LiteralExprSema::CreateTypeCheckedExpr(const TypeCheckingContext& context) const
        -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        return CreateTypeChecked(context);
    }

    auto LiteralExprSema::CreateLowered(const LoweringContext& context) const
        -> std::shared_ptr<const LiteralExprSema>
    {
        return shared_from_this();
    }

    auto LiteralExprSema::CreateLoweredExpr(const LoweringContext& context) const
    ...
    auto LiteralExprSema::Emit(Emitter& emitter) const -> ExprEmitResult
    {
        ...
        auto* const value = [&]() -> llvm::Value*
        {
            ...
            {
                return llvm::ConstantInt::get(type, (m_Kind == LiteralKind::True) ? 1 : 0);
            }
        ...
        auto* const allocaInst = emitter.GetBlock().Builder.CreateAlloca(value->getType());
    ...
    auto LiteralExprSema::GetTypeInfo() const -> TypeInfo
    {
        ...
        {
            if (m_Kind == LiteralKind::Int8)
                return natives.Int8.GetSymbol();
            else if (m_Kind == LiteralKind::Int16)
                return natives.Int16.GetSymbol();
            else if (m_Kind == LiteralKind::Int32)
                return natives.Int32.GetSymbol();
            else if (m_Kind == LiteralKind::Int64)
                return natives.Int64.GetSymbol();

            else if (m_Kind == LiteralKind::UInt8)
                return natives.UInt8.GetSymbol();
            else if (m_Kind == LiteralKind::UInt16)
                return natives.UInt16.GetSymbol();
            else if (m_Kind == LiteralKind::UInt32)
                return natives.UInt32.GetSymbol();
            else if (m_Kind == LiteralKind::UInt64)
                return natives.UInt64.GetSymbol();

            else if (m_Kind == LiteralKind::Int)
                return natives.Int.GetSymbol();

            else if (m_Kind == LiteralKind::Float32)
                return natives.Float32.GetSymbol();
            else if (m_Kind == LiteralKind::Float64)
                return natives.Float64.GetSymbol();

            else if (m_Kind == LiteralKind::String)
                return natives.String.GetSymbol();
            else if (m_Kind & LiteralKind::BoolMask)
                return natives.Bool.GetSymbol();
