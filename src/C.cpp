...
namespace Ace
{
    auto CTypes::Initialize(llvm::LLVMContext& context, llvm::Module& module) -> void
    {
    ...
    {
        auto functionCallee = module.getOrInsertFunction(name, type);
    ...
    auto CFunctions::Initialize(
        llvm::LLVMContext& context, llvm::Module& module, const Ace::CTypes& types
    ) -> void
    {
        constexpr unsigned int intBitCount = sizeof(int) * CHAR_BIT;
        constexpr unsigned int charBitCount = sizeof(char) * CHAR_BIT;
        ...
        {
            auto* const type = llvm::FunctionType::get(intType, { charPtrType }, true);

            LoadCFunction(module, &m_Printf, "printf", type);
        }

        {
            auto* const type = llvm::FunctionType::get(voidPtrType, { sizeType }, false);

            LoadCFunction(module, &m_Malloc, "malloc", type);
        }

        {
            auto* const type = llvm::FunctionType::get(voidPtrType, { sizeType, sizeType }, false);
        ...
        {
            auto* const type =
                llvm::FunctionType::get(voidPtrType, { voidPtrType, sizeType }, false);

            LoadCFunction(module, &m_Realloc, "realloc", type);
        }

        {
            auto* const type = llvm::FunctionType::get(voidPtrType, { voidPtrType }, false);
        ...
        {
            auto* const type =
                llvm::FunctionType::get(voidPtrType, { voidPtrType, intType, sizeType }, false);
        ...
        {
            auto* const type =
                llvm::FunctionType::get(voidPtrType, { voidPtrType, voidPtrType, sizeType }, false);

            LoadCFunction(module, &m_Memcpy, "memcpy", type);
        }

        {
            auto* const type = llvm::FunctionType::get(voidType, { intType }, false);
    ...
    auto C::Initialize(llvm::LLVMContext& context, llvm::Module& module) -> void
    {
