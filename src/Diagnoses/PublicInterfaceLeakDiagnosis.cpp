...
namespace Ace
{
    using VisibilityScopeMap = std::map<const ISymbol*, std::shared_ptr<const Scope>>;

    static auto IsAncestorOrSame(
        const std::shared_ptr<const Scope>& ancestor, const std::shared_ptr<const Scope>& scope
    ) -> bool
    {
        return (ancestor.get() == scope.get()) || ancestor->HasChild(scope);
    }

    static auto SelectNarrowerScope(
        const std::shared_ptr<const Scope>& lhs, const std::shared_ptr<const Scope>& rhs
    ...
    static auto
    FindContainingImpls(const FunctionSymbol* const function, const std::vector<ISymbol*>& symbols)
        -> std::vector<const ISymbol*>
    {
        ...
        std::for_each(
            ...
            {
                if (const auto* const impl = dynamic_cast<const InherentImplSymbol*>(symbol))
                {
                ...
                if (const auto* const impl = dynamic_cast<const TraitImplSymbol*>(symbol))
                {
                    if (impl->GetBodyScope() == function->GetScope())
                    {
                        impls.push_back(impl);
    ...
    static auto GetEffectiveVisibilityScope(
        const ISymbol* const symbol, const std::vector<ISymbol*>& symbols, VisibilityScopeMap& cache
    ) -> std::shared_ptr<const Scope>
    {
        ...
        if (symbol->GetAccessModifier() == AccessModifier::Priv)
        {
            ...
            const auto visibilityScope =
                optMod.has_value() ? std::shared_ptr<const Scope>{ optMod.value()->GetBodyScope() }
        ...
        if (optMod.has_value())
        {
            visibilityScope = GetEffectiveVisibilityScope(optMod.value(), symbols, cache);
        }
        ...
        if (const auto* const field = dynamic_cast<const FieldVarSymbol*>(symbol))
        {
            visibilityScope = SelectNarrowerScope(
                visibilityScope,
                GetEffectiveVisibilityScope(field->GetParentStruct(), symbols, cache)
            );
        }

        if (const auto* const prototype = dynamic_cast<const PrototypeSymbol*>(symbol))
        {
            visibilityScope = SelectNarrowerScope(
                visibilityScope,
                GetEffectiveVisibilityScope(prototype->GetParentTrait(), symbols, cache)
            );
        }

        if (const auto* const function = dynamic_cast<const FunctionSymbol*>(symbol))
        {
            ...
            std::for_each(
                ...
                [&](const ISymbol* const implSymbol)
                {
                            ...
                            dynamic_cast<const InherentImplSymbol*>(implSymbol))
                    {
                        visibilityScope = SelectNarrowerScope(
                            visibilityScope,
                            GetEffectiveVisibilityScope(impl->GetType(), symbols, cache)
                        );
                        return;
                    }

                    const auto* const impl = dynamic_cast<const TraitImplSymbol*>(implSymbol);
                    visibilityScope = SelectNarrowerScope(
                        visibilityScope,
                        GetEffectiveVisibilityScope(impl->GetType(), symbols, cache)
                    );
                    visibilityScope = SelectNarrowerScope(
                        visibilityScope,
                        GetEffectiveVisibilityScope(impl->GetTrait(), symbols, cache)
                    );
                }
            );
        }
    ...
    ) -> void
    {
            ...
            !dynamic_cast<const TraitSelfTypeSymbol*>(unaliasedType))
        {
            ...
            if (!IsAncestorOrSame(typeVisibilityScope, interfaceVisibilityScope))
            {
    ...
    ) -> Diagnosed<void>
    {
        ...
        if (const auto* const callable = dynamic_cast<const ICallableSymbol*>(symbol))
        {
            interfaceTypes.push_back(callable->GetType());
            const auto paramTypes = callable->CollectParamTypes();
            interfaceTypes.insert(end(interfaceTypes), begin(paramTypes), end(paramTypes));

            const auto* const constrained = dynamic_cast<const IConstrainedSymbol*>(symbol);
            ...
            std::for_each(
                ...
                [&](const ConstraintSymbol* const constraint)
                {
                    ...
                    interfaceTypes.insert(end(interfaceTypes), begin(traits), end(traits));
                }
            );
        }
        else if (const auto* const variable = dynamic_cast<const IVarSymbol*>(symbol))
        {
            if (dynamic_cast<const GlobalVarSymbol*>(symbol) ||
                dynamic_cast<const FieldVarSymbol*>(symbol))
            {
                interfaceTypes.push_back(variable->GetType());
            }
        }
        else if (const auto* const trait = dynamic_cast<const TraitTypeSymbol*>(symbol))
        {
        ...
        const auto interfaceVisibilityScope =
            GetEffectiveVisibilityScope(symbol, symbols, visibilityScopeCache);
        std::set<const ITypeSymbol*> visitedTypes{};
        std::set<const ITypeSymbol*> leakedTypes{};
        std::for_each(
            begin(interfaceTypes),
            end(interfaceTypes),
        ...
        std::for_each(
            ...
            [&](const ITypeSymbol* const leakedType)
            {
                diagnostics.Add(CreatePublicInterfaceLeaksPrivateTypeError(
                    symbol->GetName().SrcLocation, leakedType
                ));
            }
        );
    ...
    auto DiagnosePublicInterfaceLeaks(Compilation* const compilation) -> Diagnosed<void>
    {
        auto diagnostics = DiagnosticBag::Create();
        const auto symbols = compilation->GetGlobalScope()->CollectAllSymbolsRecursive();
        VisibilityScopeMap visibilityScopeCache{};
        ...
        std::for_each(
            ...
            [&](ISymbol* const symbol)
            {
                diagnostics.Collect(DiagnoseSymbolLeaks(symbol, symbols, visibilityScopeCache));
            }
        );
