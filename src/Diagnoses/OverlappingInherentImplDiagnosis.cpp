...
namespace Ace
{
    ...
    static auto CollectOverlappingNameImplSymbols(
        InherentImplSymbol* const lhsImpl, InherentImplSymbol* const rhsImpl
        ...
        std::for_each(
            ...
            [&](ISymbol* const lhsSymbol)
            {
                ...
                const auto matchingNameSymbolIt = std::find_if(
                    ...
                    {
                        return lhsSymbol->GetName().String == rhsSymbol->GetName().String;
                    }
                );
    ...
    ) -> std::vector<SymbolPair>
    {
        ...
        const bool couldOverlap =
            DoPlaceholdersOverlap(lhsType, rhsType) || DoPlaceholdersOverlap(rhsType, lhsType);
    ...
    static auto
    DiagnoseOverlappingImpls(InherentImplSymbol* const lhsImpl, InherentImplSymbol* const rhsImpl)
        -> Diagnosed<void>
    {
        auto diagnostics = DiagnosticBag::Create();

        const auto overlappingSymbols = CollectOverlappingImplSymbols(lhsImpl, rhsImpl);

        std::for_each(
            begin(overlappingSymbols),
            end(overlappingSymbols),
            [&](const SymbolPair& symbolPair)
            {
                diagnostics.Add(
                    CreateOverlappingInherentImplSymbolError(symbolPair.LHS, symbolPair.RHS)
                );
            }
        );
    ...
    auto DiagnoseOverlappingInherentImpls(Compilation* const compilation) -> Diagnosed<void>
    {
        ...
        for (int i = 0; i < impls.size(); i++)
        {
            ...
            {
                diagnostics.Collect(DiagnoseOverlappingImpls(impls.at(i), impls.at(j)));
            }
        }
