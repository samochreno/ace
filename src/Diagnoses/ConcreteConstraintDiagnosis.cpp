...
namespace Ace
{
    static auto DiagnoseConcreteConstraint(ConstraintSymbol* const constraint) -> Diagnosed<void>
    {
        ...
        const auto concreteTraitIt = std::find_if_not(
            ...
            {
                return trait->IsPlaceholder();
            }
        );

        const auto areAllTraitsPlaceholders = concreteTraitIt == end(constraint->GetTraits());

        const bool isValid = constraint->GetType()->IsPlaceholder() || areAllTraitsPlaceholders;

        if (!isValid)
        {
            diagnostics.Add(CreateConcreteConstraintError(constraint->GetName().SrcLocation));
        }
