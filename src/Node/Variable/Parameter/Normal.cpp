...
namespace Ace::Node::Variable::Parameter
{
    ...
    auto Normal::CloneInScope(
        const std::shared_ptr<Scope>& t_scope
    ) const -> std::shared_ptr<const Node::Variable::Parameter::Normal>
    {
        ...
        return std::make_shared<const Node::Variable::Parameter::Normal>(
            t_scope,
            m_Name,
            m_TypeName,
            clonedAttributes,
    ...
    auto Normal::CreateBound() const -> Expected<std::shared_ptr<const BoundNode::Variable::Parameter::Normal>>
    {
        ...
        auto* const selfSymbol =
            m_Scope->ExclusiveResolveSymbol<Symbol::Variable::Parameter::Normal>(m_Name).Unwrap();
