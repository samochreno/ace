...
namespace Ace::Node::Variable::Normal
{
    ...
    auto Static::CloneInScope(
        const std::shared_ptr<Scope>& t_scope
    ) const -> std::shared_ptr<const Node::Variable::Normal::Static>
    {
    ...
    auto Static::CreateBound() const -> Expected<std::shared_ptr<const BoundNode::Variable::Normal::Static>>
    {
        ...
        auto* const selfSymbol =
            m_Scope->ExclusiveResolveSymbol<Symbol::Variable::Normal::Static>(m_Name).Unwrap();
    ...
    {
        return SymbolKind::StaticVariable;
    }
