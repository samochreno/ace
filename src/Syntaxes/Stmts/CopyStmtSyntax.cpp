...
namespace Ace
{
    ...
    {
        return SyntaxChildCollector{}.Collect(m_SrcExpr).Collect(m_DstExpr).Build();
    }
    ...
    auto CopyStmtSyntax::CreateSema() const -> Diagnosed<std::shared_ptr<const CopyStmtSema>>
    {
        ...
        const auto srcExprSema = diagnostics.Collect(m_SrcExpr->CreateExprSema());
        const auto dstExprSema = diagnostics.Collect(m_DstExpr->CreateExprSema());
        ...
        return Diagnosed{
            std::make_shared<const CopyStmtSema>(
                GetSrcLocation(), typeSymbol, srcExprSema, dstExprSema
            ),
