...
namespace Ace
{
    ...
    {
        return SyntaxChildCollector{}.Collect(m_Expr).Collect(m_Args).Build();
    }
    ...
    auto CallExprSyntax::CreateSema() const -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        ...
        if (const auto* const symbolLiteralExpr =
                dynamic_cast<const SymbolLiteralExprSyntax*>(m_Expr.get()))
        {
            const auto optCallableSymbol =
                diagnostics.Collect(GetScope()->ResolveStaticSymbol<ICallableSymbol>(
                    symbolLiteralExpr->GetName(), Scope::CreateArgTypes(argTypeSymbols)
                ));
            auto* const callableSymbol =
                optCallableSymbol.value_or(GetCompilation()->GetErrorSymbols().GetCallable());

            return Diagnosed{
                std::make_shared<const StaticCallExprSema>(
                    GetSrcLocation(), GetScope(), callableSymbol, argSemas
                ),
                std::move(diagnostics),
            };
        }

        if (const auto* const memberAccessExpr =
                dynamic_cast<const MemberAccessExprSyntax*>(m_Expr.get()))
        {
            const auto exprSema =
                diagnostics.Collect(memberAccessExpr->GetExpr()->CreateExprSema());
            ...
            {
                optCallableSymbol =
                    diagnostics.Collect(GetScope()->ResolveInstanceSymbol<ICallableSymbol>(
                        selfTypeSymbol,
            ...
            return Diagnosed{
                std::make_shared<const InstanceCallExprSema>(
                    GetSrcLocation(), exprSema, callableSymbol, argSemas
                ),
                std::move(diagnostics),
            };
        }
        ...
        diagnostics.Add(
            CreateExpectedFunctionError(exprSema->GetSrcLocation(), exprSema->GetTypeInfo().Symbol)
        );
