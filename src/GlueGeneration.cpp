...
namespace Ace::GlueGeneration
{
    static auto GetOrDeclareAndBindCopyGlueSymbols(
        Compilation* const compilation, IConcreteTypeSymbol* const typeSymbol
    ) -> FunctionSymbol*
    {
        ...
        const std::string nameString =
            AnonymousIdent::Create("copy_glue", typeSymbol->CreateLocalSignature());
        const Ident name{
        ...
        auto* const glueSymbol =
            DiagnosticBag::CreateNoError().Collect(Scope::DeclareSymbol(std::move(ownedGlueSymbol))
            );
        ...
        (void)DiagnosticBag::CreateNoError().Collect(
            Scope::DeclareSymbol(std::make_unique<NormalParamVarSymbol>(
                bodyScope, selfName, dynamic_cast<ISizedTypeSymbol*>(typeSymbol->GetWithRef()), 0
            ))
        );
        ...
        (void)DiagnosticBag::CreateNoError().Collect(
            Scope::DeclareSymbol(std::make_unique<NormalParamVarSymbol>(
                bodyScope, otherName, dynamic_cast<ISizedTypeSymbol*>(typeSymbol->GetWithRef()), 1
            ))
        );
    ...
    ) -> FunctionSymbol*
    {
        ...
        const std::string nameString =
            AnonymousIdent::Create("drop_glue", typeSymbol->CreateLocalSignature());
        const Ident name{
            typeSymbol->GetName().SrcLocation,
            nameString,
        };
    ...
    static auto TryDeclareAndBindGlueSymbols(
        ...
        const std::function<FunctionSymbol*(Compilation* const, IConcreteTypeSymbol* const)>&
            getOrDeclareAndBindGlueSymbols
    ...
    static auto CreateAndBindGlueBlock(
        Compilation* const compilation,
        const std::function<std::shared_ptr<const IEmittable<void>>(
            IConcreteTypeSymbol* const, FunctionSymbol* const
        )>& createGlueBlock,
        IConcreteTypeSymbol* const typeSymbol,
    ...
    ) -> void
    {
        ...
        std::for_each(
            ...
            [&](IConcreteTypeSymbol* const typeSymbol)
            {
                const auto optGlueSymbol =
                    TryDeclareAndBindGlueSymbols(compilation, typeSymbol, getOrDeclareGlueSymbols);
                if (!optGlueSymbol.has_value())
                {
                    return;
                }

                typeGlueSymbolPairs.emplace_back(typeSymbol, optGlueSymbol.value());
            }
        );

        std::for_each(
            begin(typeGlueSymbolPairs),
            end(typeGlueSymbolPairs),
    ...
    static auto CollectTypeSymbols(Compilation* const compilation)
    ...
    auto GenerateAndBindGlue(Compilation* const compilation) -> void
    {
        ...
        GenerateAndBindGlue(
            ...
            [](IConcreteTypeSymbol* const typeSymbol, FunctionSymbol* const glueSymbol)
            {
    ...
    ) -> std::shared_ptr<const IEmittable<void>>
    {
        ...
        {
        public:
            TrivialCopyGlueBlockEmitter(IConcreteTypeSymbol* const typeSymbol)
            ...
            auto Emit(Emitter& emitter) const -> void final
            {
                ...
                auto* const otherValue = emitter.GetBlock().Builder.CreateLoad(type, otherPtr);
        ...
        return std::make_shared<const TrivialCopyGlueBlockEmitter>(structSymbol);
    }
    ...
    static auto GetCopyOpSymbol(ITypeSymbol* const structSymbol) -> std::optional<FunctionSymbol*>
    {
        ...
        const auto opSymbolIt = opMap.find(dynamic_cast<ITypeSymbol*>(structSymbol->GetRoot()));
        ...
        return dynamic_cast<FunctionSymbol*>(Scope::ForceCollectGenericInstance(opSymbol, typeArgs)
        );
    }

    static auto GetDropOpSymbol(ITypeSymbol* const structSymbol) -> std::optional<FunctionSymbol*>
    {
        auto diagnostics = DiagnosticBag::CreateNoError();
    ...
    ) -> std::shared_ptr<const IEmittable<void>>
    {
        ...
        {
            return CreateTrivialCopyGlueBlock(compilation, glueSymbol, structSymbol);
        }
        ...
        const auto selfParamRefExprSema = std::make_shared<const StaticVarRefExprSema>(
            srcLocation, blockScope, paramSymbols.at(0)
        );
        const auto otherParamRefExprSema = std::make_shared<const StaticVarRefExprSema>(
            srcLocation, blockScope, paramSymbols.at(1)
        );
        ...
        if (optOpSymbol.has_value())
        {
            ...
            const auto callExprSema = std::make_shared<const StaticCallExprSema>(
                srcLocation, blockScope, optOpSymbol.value(), args
            );

            const auto exprStmtSema =
                std::make_shared<const ExprStmtSema>(srcLocation, callExprSema);
        ...
        else
        {
            ...
            std::for_each(
                ...
                [&](FieldVarSymbol* const fieldSymbol)
                {
                    ...
                    const auto selfParamVarRefExprSema =
                        std::make_shared<const FieldVarRefExprSema>(
                            srcLocation, selfParamRefExprSema, fieldSymbol
                        );
                    const auto otherParamVarRefExprSema =
                        std::make_shared<const FieldVarRefExprSema>(
                            srcLocation, otherParamRefExprSema, fieldSymbol
                        );
                    ...
                    const auto callExprSema = std::make_shared<const StaticCallExprSema>(
                        srcLocation, blockScope, typeGlueSymbol, args
                    );
        ...
        const auto blockSema = std::make_shared<const BlockStmtSema>(
            srcLocation, blockScope->GetParent().value(), stmts
        );

        return diagnostics.Collect(
            Application::CreateVerifiedFunctionBlock(blockSema, compilation->GetVoidTypeSymbol())
        );
    }
    ...
    ) -> std::shared_ptr<const IEmittable<void>>
    {
        ...
        {
            return CreateTrivialDropGlueBlock(compilation, glueSymbol, structSymbol);
        }
        ...
        std::for_each(
            ...
            [&](FieldVarSymbol* const fieldSymbol)
            {
                auto* const typeSymbol =
                    dynamic_cast<IConcreteTypeSymbol*>(fieldSymbol->GetSizedType()->GetUnaliased());
                ACE_ASSERT(typeSymbol);
