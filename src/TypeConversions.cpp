...
namespace Ace
{
    static auto GetNativeConversionOp(
        ...
        const std::unordered_map<ITypeSymbol*, std::unordered_map<ITypeSymbol*, FunctionSymbol*>>&
            fromOpMap
    ...
    ) -> std::optional<FunctionSymbol*>
    {
        ...
        const auto isSized =
            dynamic_cast<ISizedTypeSymbol*>(pureFromType->GetUnaliased()) != nullptr;
    ...
    ) -> std::optional<FunctionSymbol*>
    {
        ...
        auto* concreteType = dynamic_cast<ISizedTypeSymbol*>(pureFromType->GetUnaliased());
        if (!concreteType)
        {
            return std::nullopt;
        }

        auto* const targetTrait = dynamic_cast<TraitTypeSymbol*>(pureTargetType->GetUnaliased());
    ...
    ) -> std::optional<FunctionSymbol*>
    {
        ...
        optConversionOp = GetStrongPtrToWeakPtrConversionOp(scope, fromType, targetType);
        if (optConversionOp.has_value())
        {
            return optConversionOp.value();
        }

        optConversionOp = GetStrongPtrToDynStrongPtrConversionOp(scope, fromType, targetType);
        if (optConversionOp.has_value())
        {
            return optConversionOp.value();
        }
    ...
    ) -> std::optional<FunctionSymbol*>
    {
        const auto optNativeOp = GetNativeConversionOp(
            fromType, targetType, scope->GetCompilation()->GetNatives().GetImplicitFromOpMap()
        );
    ...
    ) -> std::optional<FunctionSymbol*>
    {
        ...
        const auto optNativeExplicitOp = GetNativeConversionOp(
            fromType, targetType, scope->GetCompilation()->GetNatives().GetExplicitFromOpMap()
        );
    ...
    ) -> bool
    {
        ...
        for (size_t i = 0; i < fromTypeInfos.size(); i++)
        {
            const auto placeholderExpr = std::make_shared<const ConversionPlaceholderExprSema>(
                SrcLocation{ scope->GetCompilation() }, scope, fromTypeInfos.at(i)
            );

            const auto convertedExpr =
                CreateImplicitlyConverted(placeholderExpr, targetTypeInfos.at(i));
    ...
    static auto
    CreateConvertedReturn(const std::shared_ptr<const IExprSema>& expr, DiagnosticBag diagnostics)
    ...
    ) -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        ...
        if (targetTypeInfo.Symbol->IsError() || expr->GetTypeInfo().Symbol->IsError())
        {
        ...
        if (expr->GetTypeInfo().Symbol->GetUnaliased() == targetTypeInfo.Symbol->GetUnaliased())
        {
            return CreateConvertedReturn(expr, std::move(diagnostics));
        }
        ...
        if (isTargetRef && (exprValueType->GetUnaliased() != targetValueType->GetUnaliased()))
        {
            ...
            expr = std::make_shared<const RefExprSema>(
                convertedValueExpr->GetSrcLocation(), convertedValueExpr
            );
        ...
        if (isRef)
        {
            if (!isTargetRef)
            {
                expr = std::make_shared<const DerefExprSema>(expr->GetSrcLocation(), expr);
            }
        }
        else
        {
            if (isTargetRef)
            {
                expr = std::make_shared<const RefExprSema>(expr->GetSrcLocation(), expr);
            }
        }
        ...
        const auto optOpSymbol =
            function(expr->GetScope(), expr->GetTypeInfo().Symbol, targetTypeInfo.Symbol);
        if (!optOpSymbol.has_value())
        {
            diagnostics.Add(CreateUnableToConvertExprError(expr, targetTypeInfo));
            return CreateConvertedReturn(expr, std::move(diagnostics));
        }

        return CreateConvertedReturn(
            std::make_shared<const StaticCallExprSema>(
                expr->GetSrcLocation(), expr->GetScope(), optOpSymbol.value(), std::vector{ expr }
            ),
            std::move(diagnostics)
        );
    }
    ...
    ) -> Diagnosed<std::shared_ptr<const IExprSema>>
    {
        ...
        const auto convertedExpr =
            diagnostics.Collect(CreateImplicitlyConverted(expr, targetTypeInfo));
