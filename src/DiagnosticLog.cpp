...
namespace Ace
{
    ...
    static auto
    CreateHighlightString(const std::string_view string, const std::string_view highlight)
        -> std::string
    {
    ...
    static auto
    CalculateSnippetString(const std::string_view line, const std::string_view highlight)
        -> std::string_view
    {
        ...
        const size_t maxLeftPadding = std::distance(begin(line), begin(highlight));
        const size_t maxRightPadding = std::distance(end(highlight), end(line));
    ...
    static auto CalculateSnippetStringLine(const SrcLocation& srcLocation) -> std::string_view
    {
        ...
        const auto lineEndIt = std::find(srcLocation.CharacterBeginIterator, bufferEndIt, '\n');

        return std::string_view{ lineBeginIt.base(), lineEndIt };
    }

    auto CreateCappedIterator(const std::string_view line, std::string_view::const_iterator it)
    ...
    static auto CalculateSnippetDisplayInfo(const SrcLocation& srcLocation)
    ...
    {
        const auto displayInfo = CalculateSnippetDisplayInfo(diagnostic.OptSrcLocation.value());
